...
then
	echo Running Miner...
	OPTS="--datadir ${DATADIR} --rpc --rpcaddr 0.0.0.0 --port ${PORT} --rpcport ${RPC_PORT} --rpcapi ${RPC_API} --rpccorsdomain \"*\" --ipcpath ${IPC_PATH} --mine --minerthreads 1 --etherbase 0 --cache 1024 --networkid ${NETWORK_ID}"
	shift
	echo Options : $OPTS
